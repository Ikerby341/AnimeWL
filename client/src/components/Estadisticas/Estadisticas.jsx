...
import { construirFonsPastis, construirSegmentsPastis, calcularAlcadaBarra, formatarHores, formatarMinuts, formatarValor } from './statsHelpers.js';
...
function Estadistiques() {
  ...
  const pieSegments = construirSegmentsPastis(topGenres, pieColors);
  const pieBackground = construirFonsPastis(pieSegments);
    ...
    <div className="estadisticas-page">
      ...
      <>
              ...
              <div className="stats-bar-item" key={index}>
                    ...
                    style={{
                      height: calcularAlcadaBarra(item.minutes || 0, maxMinutes, minBarRatio)
                    }} />
                  
                                                <span className="stats-bar-value">{formatarHores(item.minutes)}</span>
                                            </div>
              ...
              <div className="stats-bar-empty">No hay datos suficientes para mostrar el gráfico.</div>
                    ...
                    <div className="stats-metrics">
                        <div className="stats-metric-card">
                            ...
                            <span className="stats-metric-value">{formatarMinuts(stats?.totalMinutes)}</span>
                        </div>
                        <div className="stats-metric-card">
                            <span className="stats-metric-label">Animes finalizados</span>
                            <span className="stats-metric-value">{formatarValor(stats?.totalFinishedAnimes)}</span>
                        </div>
                        <div className="stats-metric-card">
                            <span className="stats-metric-label">Capítulos vistos</span>
                            <span className="stats-metric-value">{formatarValor(stats?.totalChapters)}</span>
                        </div>
                    </div>
                </>
      }
