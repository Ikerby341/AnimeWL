...
function DetallsAnime({ anime, animeId, comments = [], commentsLoading = true, isLoggedIn = false, currentUserId = null, currentUserIsAdmin = false, onCommentAdded, onCommentDeleted, rating = { average: 0, count: 0 }, userRating = null, ratingLoading = false, ratingError = '', onRate, progress = null, progressLoading = true, progressError = '', episodeCount = 0, onProgressChange, isFavorite = false, onAddToFavorites, onRemoveFromFavorites }) {
  ...
  const gestionarClicEstrella = (value) => {
    if (!hasInteractiveRating) return;
    setSelectedStars(value);
    onRate(value);
  };

  const gestionarClicFavorit = async () => {
  ...
  const starElements = [1, 2, 3, 4, 5].map((value) => {
    ...
    return (
      <button
        ...
        onClick={() => gestionarClicEstrella(value)}
  ...
  return (
    <div className="anime-details">
            ...
            <button
              ...
              <IconaCor size={24} filled={isFavorite} />
              }
                            </button>
            }
                    </div>
