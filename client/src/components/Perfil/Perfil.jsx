import { useDadesUsuari, useAutenticacio } from './../../hooks/useAuth';
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import userIcon from './../../assets/usuari.webp';
import './Perfil.css';
import { useNotificacio } from '../../hooks/useToast.js';
...
export function Perfil({ profileUser = null, profileFavorites = [], readOnly = false, hideEmail = false }) {
  ...
  const ownUserInfo = useDadesUsuari();
  let userInfo = profileUser || ownUserInfo;
  const { checkSession } = useAutenticacio();
  const { showToast } = useNotificacio();
  ...
  useEffect(() => {
    const carregarAnime = async (id, setter, label) => {
      if (!id) {
        setter(null);
        return;
      }
    ...
    carregarAnime(userInfo?.id_anime_preferit, setFavoriteAnime, 'favorite');
    carregarAnime(userInfo?.id_anime_recomanat, setRecommendedAnime, 'recommended');
  ...
  useEffect(() => {
    ...
    const carregarAnimeEnVist = async () => {
      try {
  ...
  function normalitzarResultatAnime(anime) {
  ...
  function gestionarClicAnimePerfil(anime, editableType) {
    if (readOnly) {
      const animeId = obtenirIdAnime(anime);
      if (animeId) {
    ...
    obrirModalCerca(editableType);
  }

  function gestionarTeclaAnimePerfil(event, anime, editableType) {
    if (event.key === 'Enter' || event.key === ' ') {
      event.preventDefault();
      gestionarClicAnimePerfil(anime, editableType);
    }
  }

  async function seleccionarResultatCerca(anime) {
    if (!searchType) return;
    const normalized = normalitzarResultatAnime(anime);
    if (!normalized.id) {
  ...
  function actualitzarDadesUsuari() {
    ...
    <div className={readOnly ? 'perfil-read-only' : undefined}>
            {!readOnly && <div className='edit-pfp-container'>
                <div className='edit-pfp-overlay'>
                    ...
                    <button className='edit-pfp-button' onClick={canviarFotoPerfil}>Guardar</button>
            ...
            <div className="profile-info">
                <img src={userInfo?.img_url || userIcon} alt="Profile" className="profile-picture" onClick={mostrarEditorFotoPerfil} />
          ...
          onClick={() => gestionarClicAnimePerfil(favoriteAnime, 'favorite')}
          onKeyDown={(event) => gestionarTeclaAnimePerfil(event, favoriteAnime, 'favorite')}
          tabIndex={0}
          role="button"
          ...
          onClick={() => gestionarClicAnimePerfil(recommendedAnime, 'recommended')}
          onKeyDown={(event) => gestionarTeclaAnimePerfil(event, recommendedAnime, 'recommended')}
          tabIndex={0}
          role="button"
        ...
        <div
          ...
          onClick={() => readOnly && gestionarClicAnimePerfil(watchingAnime)}
          onKeyDown={(event) => readOnly && gestionarTeclaAnimePerfil(event, watchingAnime)}
      ...
      <div className="select-anime-container">
                        <div className="select-anime-overlay">
                            <div className="select-anime-header">
                                ...
                                <button className="close-search-button" onClick={tancarModalCerca}>Cerrar</button>
                            </div>
                            <input
          ...
          <p className="select-anime-empty">No se encontraron resultados.</p>
              ...
              const normalized = normalitzarResultatAnime(anime);
              return (
                <li key={normalized.id || anime.mal_id || anime.id} className="select-anime-result" onClick={() => seleccionarResultatCerca(anime)}>
